...
lemma fderivR_conj_of_hasDeriv
  ...
  -- Avoid rewriting `star`; prove it as a composition with `conjCLM`.
    ...
    DplusCLM (H:=H) (W:=ℂ) (fun x => star (f x)) u
        = (1/2 : ℝ) •
            ((fderivR (H:=H) (W:=ℂ) (fun x => star (f x)) u)
              - Aℒ (H:=H) (W:=ℂ) (fderivR (H:=H) (W:=ℂ) (fun x => star (f x)) u)) := rfl
    _ = (1/2 : ℝ) • ((conjCLM.comp D) - Aℒ (H:=H) (W:=ℂ) (conjCLM.comp D)) := by
          simpa [hE]
    _ = (1/2 : ℝ) • ((conjCLM.comp D) - (-(conjCLM.comp (Aℒ (H:=H) (W:=ℂ) D)))) := by
          simpa [hA]
    _ = (1/2 : ℝ) • (conjCLM.comp D + conjCLM.comp (Aℒ (H:=H) (W:=ℂ) D)) := by
          simp [sub_eq_add_neg]
    _ = conjCLM.comp ((1/2 : ℝ) • (D + Aℒ (H:=H) (W:=ℂ) D)) := by
          simp [ContinuousLinearMap.comp_add, ContinuousLinearMap.comp_smul, smul_add]
...
lemma Dminus_conj_op
  ...
  calc
    ...
    _ = (1/2 : ℝ) • (conjCLM.comp D - conjCLM.comp (Aℒ (H:=H) (W:=ℂ) D)) := by
          simp
    _ = conjCLM.comp ((1/2 : ℝ) • (D - Aℒ (H:=H) (W:=ℂ) D)) := by
          simp [ContinuousLinearMap.comp_sub, ContinuousLinearMap.comp_smul, smul_sub]
...
lemma gradPlus_conj_eq_gradMinus
  ...
  -- Use the operator identity in directional form:
  ...
  calc
    inner (𝕜:=ℂ) (gradPlus (H:=H) (fun x => star (f x)) u) v
        = DplusCLM (H:=H) (W:=ℂ) (fun x => star (f x)) u v := h₁
    _   = star (DminusCLM (H:=H) (W:=ℂ) f u v) := h₂
    _   = star (inner (𝕜:=ℂ) v (gradMinus (H:=H) f u)) := by
            rw [h₃]
    _   = inner (𝕜:=ℂ) (gradMinus (H:=H) f u) v := by
            simp
...
lemma gradMinus_conj_eq_gradPlus
  ...
  calc
    inner (𝕜:=ℂ) v (gradMinus (H:=H) (fun x => star (f x)) u)
        = DminusCLM (H:=H) (W:=ℂ) (fun x => star (f x)) u v := hL
    _   = star (DplusCLM (H:=H) (W:=ℂ) f u v) := h₁
    _   = star (inner (𝕜:=ℂ) (gradPlus (H:=H) f u) v) := by
            rw [h₂]
    _   = inner (𝕜:=ℂ) v (gradPlus (H:=H) f u) := by
