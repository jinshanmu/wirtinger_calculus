...
* Real Fréchet derivative over `ℝ` on complex Banach spaces:
  - `HasRDerivAt`, `fderivR`
...
variable {V : Type _}
variable [NormedAddCommGroup V] [Module ℂ V]
variable [InnerProductSpace ℂ V]
...
variable [NormedAddCommGroup W] [NormedSpace ℂ W]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  -- We can cancel the scalar 1/2 as it's non-zero
  suffices Jc_W ∘ D - Jc_W ∘ Aℒ D = D ∘ Jc_H - Aℒ D ∘ Jc_H by rw [this]
  -- Prove helper identities about how Jc interacts with Aℒ
  ...
  have h1 : Jc_W.comp (Aℒ D) = -D.comp Jc_H := by
    simp [Aℒ, ContinuousLinearMap.comp_assoc, Jc_W_comp_Jc_W]
  have h2 : (Aℒ D).comp Jc_H = -Jc_W.comp D := by
    simp [Aℒ, ContinuousLinearMap.comp_assoc, Jc_H_comp_Jc_H]
