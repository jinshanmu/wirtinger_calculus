...
lemma Dplus_add_Dminus (f : H → W) (u : H) :
  ...
  let D := fderivR f u
  let AD := Aℒ D
  change ((1/2 : ℝ)) • (D - AD) + ((1/2 : ℝ)) • (D + AD) = D
  rw [← smul_add]
  have h : (D - AD) + (D + AD) = (2 : ℝ) • D := by
    rw [sub_eq_add_neg, add_add_add_comm, neg_add_self, add_zero, ←two_smul]
  rw [h, smul_smul]
  rw [show (1/2 * 2 : ℝ) = 1 by norm_num, one_smul]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  have h2 : (Aℒ D).comp Jc_H = -Jc_W.comp D := by
    rw [Aℒ, ContinuousLinearMap.comp_assoc, ContinuousLinearMap.comp_assoc, Jc_H_comp_Jc_H,
      ContinuousLinearMap.comp_neg, ContinuousLinearMap.comp_id]
