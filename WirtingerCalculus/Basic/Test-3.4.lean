...
@[simp] lemma star_eq_re_sub_im (a : ℂ) :
  ...
  -- Switch to `Complex.conj` and evaluate using standard simp lemmas.
  change star a = (a.re : ℂ) - (a.im : ℂ) * I
  have h1 :
      star a = star ((a.re : ℂ) + (a.im : ℂ) * I) := by
    -- from `re_add_im : (↑a.re + ↑a.im * I) = a`
    simpa using (congrArg star (Complex.re_add_im a)).symm
  have h2 :
      star ((a.re : ℂ) + (a.im : ℂ) * I)
        = (a.re : ℂ) - (a.im : ℂ) * I := by
    -- `conj` distributes; `conj (ofReal r) = ofReal r`, `conj I = -I`
    simp [sub_eq_add_neg, mul_comm]
  simpa using h1.trans h2

/-- From `antiJ` one gets conjugate-linearity of `C` over `ℂ`.
Uses `complexSmul_decompose` defined earlier in your file. -/
lemma Conjugation.conj_smul {V} [AddCommGroup V] [Module ℂ V]
    (C : Conjugation V) (a : ℂ) (v : V) :
    C.C (a • v) = (star a) • C.C v := by
  -- decompose the complex scalar
  have h0 : a • v = (a.re : ℝ) • v + a.im • (I • v) :=
    complexSmul_decompose (V:=V) a v
  -- distribute `C`
  have h_add :
      C.C ((a.re : ℝ) • v + a.im • (I • v))
        = C.C ((a.re : ℝ) • v) + C.C (a.im • (I • v)) := by
    simpa using C.C.map_add ((a.re : ℝ) • v) (a.im • (I • v))
  have h_smul₁ : C.C ((a.re : ℝ) • v) = (a.re : ℝ) • C.C v := by
    simpa using (C.C.map_smul (a.re : ℝ) v)
  have h_smul₂ : C.C (a.im • (I • v)) = a.im • C.C (I • v) := by
    simpa using (C.C.map_smul (a.im : ℝ) (I • v))
  calc
    C.C (a • v)
        = C.C ((a.re : ℝ) • v + a.im • (I • v)) := by rw [h0]
    _   = (a.re : ℝ) • C.C v + a.im • C.C (I • v) := by
            simpa [h_smul₁, h_smul₂] using h_add
    _   = (a.re : ℝ) • C.C v + a.im • (-(I • C.C v)) := by
            simpa [Conjugation.antiJ_apply]    -- C (I•v) = -I • C v
    _   = (a.re : ℝ) • C.C v - a.im • (I • C.C v) := by
            simp [smul_neg, sub_eq_add_neg]
    _   = ((a.re : ℂ) • C.C v) - (((a.im : ℂ) * I) • C.C v) := by
            simp [Algebra.smul_def, smul_smul]
    _   = ((a.re : ℂ) - (a.im : ℂ) * I) • C.C v := by
            rw [sub_smul]
    _   = (star a) • C.C v := by
            simpa [star_eq_re_sub_im, sub_eq_add_neg]
end algebraic_J
...
/-- Make the apply-form definitional for `simp`. -/
...
@[simp] lemma inner_adjoint_linear (A : H →L[ℂ] W) (x : W) (y : H) :
  ...
  -- `adjoint_inner_left` with arguments swapped gives
  --   ⟪(A†) x, y⟫ = ⟪x, A y⟫
  -- so taking `.symm` matches the goal exactly.
  simpa using
    (ContinuousLinearMap.adjoint_inner_left (A := A) (x := y) (y := x)).symm
...
lemma Conjugation.isometry_to_antiunitary
  ...
  have hxIy  : C.C (x - I • y) = C.C x + I • C.C y := by
    -- `C (I•y) = star I • C y = -I • C y` ⇒ `C (x - I•y) = C x + I • C y`
  ...
  -- Expand polarization for ⟪x, y⟫ on the right.
  ...
  set n :
      ℂ :=
      (↑‖x + y‖ ^ 2 - ↑‖x - y‖ ^ 2
        + (↑‖x + I • y‖ ^ 2 - ↑‖x - I • y‖ ^ 2) * I) with hn
  set m :
      ℂ :=
      (↑‖x + y‖ ^ 2 - ↑‖x - y‖ ^ 2
        + (↑‖x - I • y‖ ^ 2 - ↑‖x + I • y‖ ^ 2) * I) with hm
  ...
  have num_eq : n = star m := by
    ...
    have hstar :
      ...
      simp [hm, star_add, star_sub, star_mul, star_pow, Complex.conj_ofReal, conj_I,
            sub_eq_add_neg, add_comm, add_left_comm, add_assoc,
            mul_comm, mul_left_comm, mul_assoc]
    -- Turn `… * (-I)` into `… * I` with the swapped difference.
    have hflip :
        (↑‖x - I • y‖ ^ 2 - ↑‖x + I • y‖ ^ 2) * (-I)
          = (↑‖x + I • y‖ ^ 2 - ↑‖x - I • y‖ ^ 2) * I := by
      ring
  ...
  have : n / (4:ℂ) = star (m / (4:ℂ)) := by
    -- `star (m/4) = (star m)/4`
    have hsd : star (m / (4:ℂ)) = (star m) / (4:ℂ) := by
      simpa using (star_div m (4:ℂ))
