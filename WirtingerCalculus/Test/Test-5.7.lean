...
/-- Canonical, parametric inverse chain rule over `ℝ`:
If `hg : HasRDerivAt g u Dg` and `hgu : g u ≠ 0`, then for **any** `c : ℂ` with
`HasFDerivAt (fun z : ℂ => z⁻¹) ((1 : ℂ →L[ℂ] ℂ).smulRight c) (g u)`,
we have the `ℝ`-Fréchet derivative for `x ↦ (g x)⁻¹` in the cheap
`restrictScalars` form.  (We pick the concrete `c` only when needed.) -/
lemma hasRDerivAt_inv_from_hasDeriv
  ...
  HasRDerivAt (fun x => (g x)⁻¹) u
    ((((1 : ℂ →L[ℂ] ℂ).smulRight c).restrictScalars ℝ).comp Dg) := by
  -- view the ℂ-derivative over ℝ and compose with `g`
...
lemma gradMinus_inv
  ...
  have h_inv : HasRDerivAt (fun x => (g x)⁻¹) u
      ((mulLeftCLM ( - ((g u)⁻¹ * (g u)⁻¹))).comp Dg) :=
    hasRDerivAt_inv_from_hasDeriv (H:=H) (u:=u) (Dg:=Dg) hg hgu
  have hD :
      DminusCLM (H:=H) (W:=ℂ) (fun x => (g x)⁻¹) u
        = (mulLeftCLM ( -((g u)⁻¹ * (g u)⁻¹))).comp (DminusCLM (H:=H) (W:=ℂ) g u) := by
    unfold DminusCLM
    simp [fderivR, hg.fderiv, h_inv.fderiv]
  apply (InnerProductSpace.toDual ℂ H).injective; ext v
  have hv :
      inner (𝕜 := ℂ) v (gradMinus (H:=H) (fun x => (g x)⁻¹) u)
        = - ((g u)⁻¹ * (g u)⁻¹) * inner (𝕜 := ℂ) v (gradMinus (H:=H) g u) := by
    simp [Dminus_eq_inner_gradMinus, hD, ContinuousLinearMap.comp_apply, mulLeftCLM_apply]
  -- put the scalar on the left, in the exact shape used by the tests
...
lemma gradPlus_inv
  ...
  have hD :
      DplusCLM (H:=H) (W:=ℂ) (fun x => (g x)⁻¹) u
        = (mulLeftCLM ( -((g u)⁻¹ * (g u)⁻¹))).comp (DplusCLM (H:=H) (W:=ℂ) g u) := by
    unfold DplusCLM
    simp [fderivR, hg.fderiv, h_inv.fderiv]
  apply (InnerProductSpace.toDual ℂ H).injective; ext v
  have hv :
    ...
    simp [inner_gradPlus_eq_Dplus, hD, ContinuousLinearMap.comp_apply, mulLeftCLM_apply]
  -- move scalar from the right slot to the left slot; take `star` to produce conjugates
  have h★ := congrArg star hv
  -- rewrite into the exact scalar-on-the-left shape that tests expect
  simpa [inner_conj_symm, star_mul, star_inv, map_neg,
        mul_comm, mul_left_comm, mul_assoc, inner_smul_left] using h★

/-- Quotient rule for `∇₊` (product-of-inverses shape), assuming `g[u] ≠ 0`. -/
lemma gradPlus_div
  {f g : H → ℂ} {u : H} {Df Dg : H →L[ℝ] ℂ}
  ...
  calc
    ...
    _   = (star (f u)) • gradPlus (H:=H) (fun x => (g x)⁻¹) u
            + (star ((g u)⁻¹)) • gradPlus (H:=H) f u := by
            simpa using hmul
    _   = (star (f u)) • ( - ((star (g u))⁻¹ * (star (g u))⁻¹) • gradPlus (H:=H) g u )
            + ((star (g u))⁻¹) • gradPlus (H:=H) f u := by
            simpa [hinv, star_inv]
    _   = ((star (g u))⁻¹ * (star (g u))⁻¹) •
            ( (star (g u)) • gradPlus (H:=H) f u
              - (star (f u)) • gradPlus (H:=H) g u ) := by
      simp [smul_add, smul_smul, sub_eq_add_neg, mul_comm]
...
lemma gradMinus_div
  ...
  calc
    ...
    _   = (f u) • gradMinus (H:=H) (fun x => (g x)⁻¹) u
            + ((g u)⁻¹) • gradMinus (H:=H) f u := by
            simpa using hmul
    _   = (f u) • ( - ((g u)⁻¹ * (g u)⁻¹) • gradMinus (H:=H) g u )
            + ((g u)⁻¹) • gradMinus (H:=H) f u := by
            simpa [hinv]
    _   = ((g u)⁻¹ * (g u)⁻¹) •
            ( (g u) • gradMinus (H:=H) f u
...
/-- Left-slot inner form for `∇₋(1/g)`:
`inner (∇₋(1/g) u) v
  = -((conj (g u))⁻¹ * (conj (g u))⁻¹ * inner (∇₋g u) v)`. -/
@[simp]
lemma inner_gradMinus_inv_left
  {g : H → ℂ} {u : H} {Dg : H →L[ℝ] ℂ}
  ...
  -- `hv` gives: `= inner (∇₋g u) v * star(-((g u)⁻¹ * (g u)⁻¹))`
  -- push the scalar to the left, expand stars, and arrange factors
  simpa [inner_smul_left_comm, star_mul, star_inv, map_neg,
        mul_comm, mul_left_comm, mul_assoc] using hv

/-- Left-slot inner form for `∇₊(1/g)`:
`inner (∇₊(1/g) u) v
  = -(g u)⁻¹ * (g u)⁻¹ * inner (∇₊g u) v)`. -/
@[simp]
lemma inner_gradPlus_inv_left
  {g : H → ℂ} {u : H} {Dg : H →L[ℝ] ℂ}
  (hg : HasRDerivAt g u Dg) (hgu : g u ≠ 0) (v : H) :
  inner (𝕜 := ℂ) (gradPlus (H:=H) (fun x => (g x)⁻¹) u) v
    = -((g u)⁻¹ * (g u)⁻¹) * inner (𝕜 := ℂ) (gradPlus (H:=H) g u) v := by
  ...
  -- hv : ⟪∇₊(1/g), v⟫ = ⟪∇₊g, v⟫ * (-(g u)⁻¹ * (g u)⁻¹)
  -- put the scalar on the left to match the stated shape
  simpa [mul_comm, mul_left_comm, mul_assoc, inner_smul_left_comm,
         star_mul, star_inv, map_neg] using hv
