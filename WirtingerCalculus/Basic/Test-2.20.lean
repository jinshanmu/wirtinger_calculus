...
This file formalizes the LaTeX “Setup and Notation” text:

• Real Fréchet derivative over ℝ on complex Hilbert spaces (Eq. (1)) and its
  directional/Gâteaux form (Eq. (2))
• Complex structure `J`, conjugations, and relations `J ∘ J = -id`, `C ∘ J = - J ∘ C`
• Wirtinger operators `A`, `D₊`, `D₋` and the split `Df = D₊ f + D₋ f` (Eqs. (3)–(4))
• Riesz representation for the scalar case and “Wirtinger gradients” ∇₊, ∇₋ (Eqs. (5)–(6))
• Hermitian adjoints for complex-linear maps (Eq. (7)) via `ContinuousLinearMap.adjoint`
...
/-! ## Complex structure `J` on a complex module (ℝ-linear) -/
...
/-! ## The sandwich `A` and ℝ-linear Wirtinger split -/

/-- `A T = J_W ∘ T ∘ J_H`. -/
@[simp] def A (T : H →ₗ[ℝ] W) : H →ₗ[ℝ] W :=
  (J_W).comp (T.comp J_H)
...
/-- Split: `T₊ + T₋ = T`. (Eq. (4) at the ℝ-linear map level) -/
...
/-! ## Conjugations and their properties -/
...
/-! ## Standard inner-product symmetry: `star ⟪x,y⟫ = ⟪y,x⟫` (used in Eq. (9)) -/
section
...
/-! ## Real Fréchet derivative over ℝ and continuous `J` (Eq. (1)) -/
...
/-- Continuous version of `J` (as a continuous ℝ-linear map). -/
...
/-- Inner-product identity for the conjugate-linear adjoint (unstarred form). -/
...
/-- Starred version: `⟪x, B y⟫ = star ⟪(conjAdjoint B) x, y⟫`. (Eq. (8)) -/
...
/-- Vector-level identity: `a • v = a.re • v + a.im • I • v`. -/
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- Use `isCLinearR_Dplus` to get `D (I • v) = I • D v`
      have hI : D (I • v) = I • D v := by
        ...
        exact h0.symm
      -- Decompose `a • v` and push through `D`
      ...
      -- Convert ℝ-smuls in codomain to ℂ-multiplication and factor
      have hR_mul :
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      have hI_G : I * G v = G (I • v) := by
        ...
        simp [h_star, mul_comm]
      -- Decompose and push through `G`
      have hv := complexSmul_decompose a v
...
@[simp] lemma Dminus_eq_inner_gradMinus (f : H → ℂ) (u v : H) :
  ...
  calc
    ...
    _   = inner (𝕜 := ℂ) v (gradMinus f u) := by rw [inner_star_eq_swap]
...
@[simp] lemma lineCLM_apply (v : H) (t : ℝ) : lineCLM v t = t • v := by
  -- `smulRight_apply` gives `(A.smulRight v) x = A x • v`.
  simp [lineCLM]
