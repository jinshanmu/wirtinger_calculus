...
@[simp] lemma smulRight_id_restrict_eq_mulLeft (c : ℂ) :
  ...
  ext z; simp [Algebra.smul_def, mul_comm]
...
lemma gradPlus_inv
  ...
  have hD :
    ...
    -- expand D₊ on the operator `(mulLeftCLM c).comp Dg`
    -- using the commuting rules for `Aℒ` and composition.
    unfold DplusCLM
    simp [fderivR, hg.fderiv, h_inv.fderiv, Aℒ_comp_mulLeftCLM,
          ContinuousLinearMap.comp_sub, ContinuousLinearMap.comp_smul,
          ContinuousLinearMap.smul_comp, sub_eq_add_neg, smul_add]
...
lemma gradMinus_inv
  ...
  -- same scalar, but now in the second slot (no `star`).
