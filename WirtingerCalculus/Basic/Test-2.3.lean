...
lemma Conjugation.isometry (hC : C.IsAntiunitary) : Isometry C.C := by
  ...
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    ...
    rw [← inner_self_re_eq_norm_sq, hinner, inner_self_re_eq_norm_sq]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  have h_JAL : (Jc_W).comp (Aℒ (H:=H) (W:=W) D) = - D.comp Jc_H := by
    ext x; dsimp [Aℒ]; rw [ContinuousLinearMap.comp_apply, ContinuousLinearMap.comp_apply, Jc_comp_Jc_apply]
  -- helper identity for Aℒ ∘ Jc_H
  have h_AJ : (Aℒ (H:=H) (W:=W) D).comp Jc_H = - (Jc_W).comp D := by
    ext x; dsimp [Aℒ]; rw [ContinuousLinearMap.comp_apply, ContinuousLinearMap.comp_apply, Jc_comp_Jc_apply, map_neg]
  -- compute both sides
...
{ toLinearMap :=
  { toFun := fun v => DplusCLM (H:=H) (W:=ℂ) f u v
    ...
    , map_smul' := by
        ...
        let a := c.re
        let b := c.im
        have hc : c = (a : ℂ) + (b : ℂ) * I := (re_add_im c).symm
        have h_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
          simp only [hc, add_smul, smul_assoc, real_smul_eq_coe_smul]
        calc D (c • v)
            = D ((a : ℝ) • v + (b : ℝ) • (I • v)) := by rw [h_decomp]
          _ = (a : ℝ) • D v + (b : ℝ) • D (I • v)   := by simp [map_add]
          _ = (a : ℝ) • D v + (b : ℝ) • (I • D v)   := by rw [hJ]
          _ = ((a : ℂ) • D v) + ((b : ℂ) • I • D v) := by simp [real_smul_eq_coe_smul]
...
{ toFun := fun v => star (DminusCLM (H:=H) (W:=ℂ) f u v)
  ...
  , map_smul' := by
      ...
      have h_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
          simp only [hc, add_smul, smul_assoc, real_smul_eq_coe_smul]
      calc star (D (c • v))
          = star (D ((a : ℝ) • v + (b : ℝ) • (I • v))) := by rw [h_decomp]
        _ = star ((a : ℝ) • D v + (b : ℝ) • D (I • v)) := by simp [map_add]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (D (I • v)) := by simp [star_add]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (-I • D v) := by rw [hJ]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • (I • star (D v)) := by
            simp [star_smul, star_I, neg_neg]
        _ = ((a : ℂ) + (b : ℂ) * I) • star (D v) := by
            rw [real_smul_eq_coe_smul, real_smul_eq_coe_smul, smul_assoc, ← add_smul]
        _ = c • star (D v) := by rw [hc]
