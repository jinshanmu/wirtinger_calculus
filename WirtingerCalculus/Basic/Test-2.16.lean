...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- ℂ-scalar action decomposition and linearity
      calc
        D (a • v)
            = D (a.re • v + a.im • (I • v)) := by
                simpa [Complex.smul_def]
        _   = D (a.re • v) + D (a.im • (I • v)) := by
                simp [map_add]
        _   = a.re • D v + a.im • D (I • v) := by
                simp [D.map_smul]
        _   = a.re • D v + a.im • (I • D v) := by
                simpa [hI]
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- Work via an ℝ-CLM `G = conj ∘ D`
      let G : H →L[ℝ] ℂ := Complex.conjCLM.comp D
      have G_apply : ∀ w, G w = star (D w) := by
        intro w; rfl
      -- From anti-commutation: I • D v = - D (I • v) ⇒ D (I • v) = - I • D v
      ...
      have hI_G : I • G v = G (I • v) := by
        -- On ℂ, `•` is multiplication; show in multiplicative form:
        change I * G v = G (I • v)
        simp [G, ContinuousLinearMap.comp_apply, Complex.smul_def, hI_D]
      -- Now ℂ-linearity via decomposition
      have hCL :
          G (a • v) = a • G v := by
        calc
          G (a • v)
              = G (a.re • v + a.im • (I • v)) := by
                  simpa [Complex.smul_def]
          _   = G (a.re • v) + G (a.im • (I • v)) := by
                  simp [map_add]
          _   = a.re • G v + a.im • G (I • v) := by
                  simp [G, (Complex.conjCLM.comp D).map_smul]
          _   = a.re • G v + a.im • (I • G v) := by
                  simpa [hI_G]
          _   = ((a.re : ℂ) + (a.im : ℂ) * I) • G v := by
                  simp [add_smul, mul_smul]
          _   = a • G v := by
                  simpa [Complex.re_add_im a]
      -- Unfold G back to `star ∘ D`
      simpa [G_apply] using hCL } ,
