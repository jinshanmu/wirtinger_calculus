...
lemma Dplus_add_Dminus (f : H → W) (u : H) :
  ...
  simp_rw [DplusCLM, DminusCLM, ← smul_add]
  abel_nf
  rw [smul_smul, one_div_two_mul_two, one_smul]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  have h2 : (Aℒ D).comp Jc_H = -Jc_W.comp D := by
    rw [Aℒ, ContinuousLinearMap.comp_assoc, ContinuousLinearMap.comp_assoc, Jc_H_comp_Jc_H]; simp
