...
/-! ## Formal Partials (H = W = ℂ) -/
section formal_partials

open Complex

/-- The formal partial `f_u` at `u` (in the one-dimensional case `H = ℂ`):
it is the scalar so that `D₊ f[u] v = f_u[u] * v`. -/
def formalDu (f : ℂ → ℂ) (u : ℂ) : ℂ :=
  DplusCLM (H := ℂ) (W := ℂ) f u 1

/-- The formal partial `f_{\overline u}` at `u` (in the one-dimensional case `H = ℂ`):
it is the scalar so that `D₋ f[u] v = f_{\overline u}[u] * (conj v)`. -/
def formalDconj (f : ℂ → ℂ) (u : ℂ) : ℂ :=
  DminusCLM (H := ℂ) (W := ℂ) f u 1

@[simp] lemma formalDu_def (f : ℂ → ℂ) (u : ℂ) :
    formalDu f u = DplusCLM (H := ℂ) (W := ℂ) f u 1 := rfl

@[simp] lemma formalDconj_def (f : ℂ → ℂ) (u : ℂ) :
    formalDconj f u = DminusCLM (H := ℂ) (W := ℂ) f u 1 := rfl

/-- On `ℂ`, the `ℂ`-linear map `D₊ f[u]` is left multiplication by `formalDu f u`. -/
lemma DplusCLM_scalar (f : ℂ → ℂ) (u v : ℂ) :
  DplusCLM (H := ℂ) (W := ℂ) f u v = (formalDu f u) * v := by
  have hv : (v : ℂ) • (1 : ℂ) = v := by simp
  calc
    DplusCLM (H := ℂ) (W := ℂ) f u v
        = (DplusCLM_c_linear (H := ℂ) f u) v := rfl
    _   = (DplusCLM_c_linear (H := ℂ) f u) (v • (1 : ℂ)) := by simp
    _   = v • (DplusCLM_c_linear (H := ℂ) f u) 1 := by
          simpa using (DplusCLM_c_linear (H := ℂ) f u).map_smul v 1
    _   = v * DplusCLM (H := ℂ) (W := ℂ) f u 1 := by
          simp [DplusCLM_c_linear_apply]
    _   = (formalDu f u) * v := by
          simp [formalDu, mul_comm]

/-- On `ℂ`, the `ℂ`-antilinear map `D₋ f[u]` has the form
`v ↦ (formalDconj f u) * (conj v)`. -/
lemma DminusCLM_scalar (f : ℂ → ℂ) (u v : ℂ) :
  DminusCLM (H := ℂ) (W := ℂ) f u v = (formalDconj f u) * star v := by
  -- The map `L v := star (D₋ f[u] v)` is `ℂ`-linear. For a 1D `ℂ`-space,
  -- any `ℂ`-linear map satisfies `L v = v * L 1`.
  have hL :
      star (DminusCLM (H := ℂ) (W := ℂ) f u v)
        = v * star (DminusCLM (H := ℂ) (W := ℂ) f u 1) := by
    simpa [DminusCLM_star_c_linear_apply, Algebra.smul_def]
      using (DminusCLM_star_c_linear (H := ℂ) f u).map_smul v (1 : ℂ)
  -- Take conjugates and simplify.
  have h' :
      DminusCLM (H := ℂ) (W := ℂ) f u v
        = DminusCLM (H := ℂ) (W := ℂ) f u 1 * star v := by
    simp
  -- Identify `D₋ f[u] 1` with `formalDconj f u` and commute the factors.
  simp [formalDconj, mul_comm]

/-- Helper: two `ℂ`-linear maps `ℂ →L[ℂ] ℂ` are equal if they agree at `1`. -/
lemma clinear_eq_of_eq_one {L M : ℂ →L[ℂ] ℂ} (h : L 1 = M 1) : L = M := by
  apply ContinuousLinearMap.ext
  intro z
  have hz : (z : ℂ) • (1 : ℂ) = z := by simp
  calc
    L z = L (z • 1) := by simp
    _   = z • L 1 := by simpa using L.map_smul z (1 : ℂ)
    _   = z • M 1 := by simp [h]
    _   = M (z • 1) := (M.map_smul z (1 : ℂ)).symm
    _   = M z := by simp

/-- `∇₊ f[u] = \overline{f_u[u]}` in the one-dimensional case `H = ℂ`. -/
lemma gradPlus_eq_conj_formalDu (f : ℂ → ℂ) (u : ℂ) :
  gradPlus (H := ℂ) f u = star (formalDu f u) := by
  -- compare the Riesz images and check equality at `1`
  apply (InnerProductSpace.toDual ℂ ℂ).injective
  refine clinear_eq_of_eq_one ?_
  have hL :
      ((InnerProductSpace.toDual ℂ ℂ) (gradPlus (H := ℂ) f u)) 1
        = DplusCLM (H := ℂ) (W := ℂ) f u 1 := by
    simp [gradPlus]
  have h11 :
      inner (𝕜 := ℂ) ((star (formalDu f u)) • (1 : ℂ)) (1 : ℂ)
        = inner (𝕜 := ℂ) (1 : ℂ) (1 : ℂ) * (formalDu f u) := by
    simp
  have h12 :
      inner (𝕜 := ℂ) (1 : ℂ) (1 : ℂ) = (1 : ℂ) := by
    simp
  have hR :
      ((InnerProductSpace.toDual ℂ ℂ) (star (formalDu f u))) 1
        = formalDu f u := by
    -- rewrite `((⋆ f_u) • 1)` to `⋆ f_u` via `simp`, then use `h12`
    simp
  simpa [formalDu] using hL.trans hR.symm

/-- `∇₋ f[u] = f_{\overline u}[u]` in the one-dimensional case `H = ℂ`. -/
lemma gradMinus_eq_formalDconj (f : ℂ → ℂ) (u : ℂ) :
  gradMinus (H := ℂ) f u = formalDconj f u := by
  -- again compare Riesz images and check equality at `1`
  apply (InnerProductSpace.toDual ℂ ℂ).injective
  refine clinear_eq_of_eq_one ?_
  have hL :
      ((InnerProductSpace.toDual ℂ ℂ) (gradMinus (H := ℂ) f u)) 1
        = star (DminusCLM (H := ℂ) (W := ℂ) f u 1) := by
    simp [gradMinus, DminusCLM_star_c_linear_apply]
  have hR :
      ((InnerProductSpace.toDual ℂ ℂ) (formalDconj f u)) 1
        = star (formalDconj f u) := by
    -- `⟪formalDconj, 1⟫ = star (formalDconj)`
    have : inner (𝕜 := ℂ) ((formalDconj f u) • (1 : ℂ)) (1 : ℂ)
        = inner (𝕜 := ℂ) (1 : ℂ) (1 : ℂ) * star (formalDconj f u) := by
      simp
    have h12 :
        inner (𝕜 := ℂ) (1 : ℂ) (1 : ℂ) = (1 : ℂ) := by
      simp
...
/-- The two “formal partials” identities packaged together. -/
lemma formal_partials_identities (f : ℂ → ℂ) (u : ℂ) :
  gradPlus (H := ℂ) f u = star (formalDu f u)
  ∧ gradMinus (H := ℂ) f u = formalDconj f u :=
⟨gradPlus_eq_conj_formalDu (f := f) (u := u),
 gradMinus_eq_formalDconj (f := f) (u := u)⟩
