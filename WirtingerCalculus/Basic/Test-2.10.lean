...
-- Small helpers to move between ℝ- and ℂ-scalar actions without heavy `simp`.
section smul_helpers
  ...
  @[simp] lemma ofReal_mul_I_smul (r : ℝ) (v : V) :
      ((r : ℂ) * I) • v = r • (I • v) := by
    calc
      ((r : ℂ) * I) • v = (r : ℂ) • (I • v) := by simpa [smul_smul]
      _ = (r : ℝ) • (I • v) := rfl
end smul_helpers
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- From commutation with J: (Jc ℂ) ∘ D = D ∘ Jc_H
      have h0 :=
        congrArg (fun (L : H →L[ℝ] ℂ) => L v)
          (isCLinearR_Dplus (H:=H) (W:=ℂ) f u)
      -- Normalize `h0` pointwise, then rewrite both sides
      change (Jc ℂ) (D v) = D (Jc_H v) at h0
      have hJ : D (I • v) = I • D v := by
        simpa [Jc_apply] using h0.symm
      -- Split `a` and compute only with ℝ-linearity + the `hJ` fact.
      have ha : a = (a.re : ℂ) + (a.im : ℂ) * I := by
        simpa [mul_comm] using (Complex.re_add_im a).symm
      -- Rewrite the argument of `D` into a sum of two ℝ-smuls.
      have hA : ((a.re : ℂ) • v : H) = (a.re : ℝ) • v := rfl
      have hB : (((a.im : ℂ) * I) • v : H) = (a.im : ℝ) • (I • v) := by
        calc
          ((a.im : ℂ) * I) • v = (a.im : ℂ) • (I • v) := by
            simpa [smul_smul]
          _ = (a.im : ℝ) • (I • v) := rfl
      -- Now push `D` across add and ℝ-smul.
      calc
        D (a • v)
        ...
        _   = (a.re : ℝ) • D v + (a.im : ℝ) • (I • D v) := by
                simpa [hJ]
        _   = a • D v := by
          -- Turn ℝ-smul on ℂ into multiplication, factor and recombine.
          have : (a.re : ℝ) • D v + (a.im : ℝ) • (I • D v)
               = (a.re : ℂ) * D v + (a.im : ℂ) * ((I : ℂ) * D v) := by
            simp [Algebra.smul_def]
          calc
            (a.re : ℝ) • D v + (a.im : ℝ) • (I • D v)
                = (a.re : ℂ) * D v + (a.im : ℂ) * ((I : ℂ) * D v) := by
            ...
            _ = ((a.re : ℂ) + (a.im : ℂ) * I) * D v := by
                  calc
                    ...
                    _ = ((a.re : ℂ) + (a.im : ℂ) * I) * D v := by
                            simpa [add_mul]
            _ = a * D v := by simpa [ha, mul_comm, mul_left_comm, mul_assoc] } ,
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- From anti-commutation: (Jc ℂ) ∘ D = - D ∘ Jc_H
      have h0 :=
        congrArg (fun (L : H →L[ℝ] ℂ) => L v)
          (isALinearR_Dminus (H:=H) (W:=ℂ) f u)
      change (Jc ℂ) (D v) = - (D (Jc_H v)) at h0
      -- Rearranged: D (I • v) = - I • D v
      have hI : D (I • v) = - I • D v := by
        have : (I : ℂ) • D v = - D (I • v) := by
          simpa [Jc_apply] using h0
      ...
      have hB : (((a.im : ℂ) * I) • v : H) = (a.im : ℝ) • (I • v) := by
        calc
          ((a.im : ℂ) * I) • v = (a.im : ℂ) • (I • v) := by
            simpa [smul_smul]
          _ = (a.im : ℝ) • (I • v) := rfl
      -- Push `D` across and take stars carefully
      calc
        star (D (a • v))
            = star (D (((a.re : ℂ) + (a.im : ℂ) * I) • v)) := by
                simpa [ha]
        ...
        _   = a • star (D v) := by
          -- as in the `Dplus` case: turn ℝ-smul into multiplication and factor
          have : (a.re : ℝ) • star (D v) + (a.im : ℝ) • (I • star (D v))
               = (a.re : ℂ) * star (D v) + (a.im : ℂ) * ((I : ℂ) * star (D v)) := by
            simp [Algebra.smul_def]
          calc
            (a.re : ℝ) • star (D v) + (a.im : ℝ) • (I • star (D v))
                = (a.re : ℂ) * star (D v) + (a.im : ℂ) * (I * star (D v)) := by
                      simpa [Algebra.smul_def] using this
            _ = ((a.re : ℂ) + (a.im : ℂ) * I) * star (D v) := by
                  calc
                    (a.re : ℂ) * _ + (a.im : ℂ) * (I * _)
                        = (a.re : ℂ) * _ + ((a.im : ℂ) * I) * _ := by
                            simpa [mul_assoc]
                    _ = ((a.re : ℂ) + (a.im : ℂ) * I) * _ := by
                            simpa [add_mul]
            _ = a • star (D v) := by
                  simpa [ha, Algebra.smul_def, mul_comm, mul_left_comm, mul_assoc] } ,
