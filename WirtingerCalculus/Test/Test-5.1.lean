...
variable {H : Type u}
variable [NormedAddCommGroup H] [InnerProductSpace ℂ H]
-- (No `CompleteSpace H` needed for the purely operator-level lemmas below.)
...
@[simp] lemma Aℒ_smul_complex (c : ℂ) (T : H →L[ℝ] ℂ) :
    Aℒ (H:=H) (W:=ℂ) (c • T) = c • Aℒ (H:=H) (W:=ℂ) T := by
  -- rewrite `c • T` as composition with `mulLeftCLM c`
...
lemma Dplus_const_mul
  ...
  simp [DplusCLM, fderivR_const_mul (H:=H) (u:=u) (c:=c) hf,
        hDf, mulLeftCLM_comp_eq_smul, Aℒ_smul_complex, sub_eq_add_neg, sub_eq_add_neg, smul_add]
...
lemma Dminus_const_mul
  ...
  simp [DminusCLM, fderivR_const_mul (H:=H) (u:=u) (c:=c) hf,
        hDf, ContinuousLinearMap.comp_add, mulLeftCLM_comp_eq_smul,
         Aℒ_smul_complex, smul_add]
...
lemma Dplus_mul
  ...
  simp [DplusCLM, hmul, ContinuousLinearMap.comp_add, add_comm, add_left_comm, add_assoc,
...
lemma Dminus_mul
  ...
  have hmul :
    ...
    simpa [fderivR, mulLeftCLM_comp_eq_smul, Pi.mul_def, hf.fderiv, hg.fderiv]
      using (HasFDerivAt.fderiv (HasFDerivAt.mul hf hg))
  simp [DminusCLM, hmul, ContinuousLinearMap.comp_add, add_comm, add_left_comm, add_assoc, smul_add]
...
lemma gradMinus_linear_comb
  ...
  -- take conjugates to flip slots and push scalars to the FIRST slot
  have base' := congrArg star base
  simpa [star_add, star_mul, inner_conj_symm, inner_add_left, inner_smul_left] using base'
...
private lemma conj_ne_zero_of_ne_zero {z : ℂ} (hz : z ≠ 0) : star z ≠ 0 := by
  intro h
  exact hz (by simpa using congrArg star h)
...
lemma gradPlus_div
  ...
  apply (InnerProductSpace.toDual ℂ H).injective
  ext v
  have hz : star (g u) ≠ 0 := conj_ne_zero_of_ne_zero hgu
  have hstarInv : star ((fun x => (g x)⁻¹) u) = (star (g u))⁻¹ := by simpa
  -- unfold both sides via inner
  have LHS :
      inner (𝕜 := ℂ) (gradPlus (H:=H) (fun x => f x / g x) u) v
        = (star (f u)) * inner (𝕜 := ℂ) (gradPlus (H:=H) (fun x => (g x)⁻¹) u) v
          + (star (g u))⁻¹ * inner (𝕜 := ℂ) (gradPlus (H:=H) f u) v := by
    simpa [hfg, hmul, hstarInv, inner_add_left, inner_smul_left]
  have R1 :
      inner (𝕜 := ℂ) (gradPlus (H:=H) (fun x => (g x)⁻¹) u) v
        = - ((star (g u))^2)⁻¹ * inner (𝕜 := ℂ) (gradPlus (H:=H) g u) v := by
    simpa [hinv, inner_smul_left, pow_two, inv_pow]
  have RHS :
      inner (𝕜 := ℂ)
        (((star (g u))^2)⁻¹ •
          ((star (g u)) • gradPlus (H:=H) f u - (star (f u)) • gradPlus (H:=H) g u)) v
        =
        (star (g u))⁻¹ * inner (𝕜 := ℂ) (gradPlus (H:=H) f u) v
        + -(star (f u)) * ((star (g u))^2)⁻¹ * inner (𝕜 := ℂ) (gradPlus (H:=H) g u) v := by
    -- expand RHS
    have := by
      simp [inner_smul_left, inner_add_left, sub_eq_add_neg,
            pow_two, inv_pow, mul_comm, mul_left_comm, mul_assoc,
            inv_mul_cancel hz, mul_inv_cancel hz]
    exact this
  -- put pieces together
  simpa [LHS, R1, add_comm, add_left_comm, add_assoc, mul_comm, mul_left_comm, mul_assoc] using RHS
...
lemma gradMinus_div
  ...
  apply (InnerProductSpace.toDual ℂ H).injective
  ext v
  have h1 : ((fun x => (g x)⁻¹) u) = (g u)⁻¹ := by simp
  have hz : g u ≠ 0 := hgu
  have LHS :
      inner (𝕜 := ℂ) v (gradMinus (H:=H) (fun x => f x / g x) u)
        = (g u)⁻¹ * inner (𝕜 := ℂ) v (gradMinus (H:=H) f u)
          + (f u) * inner (𝕜 := ℂ) v (gradMinus (H:=H) ((fun x => (g x)⁻¹)) u) := by
    simpa [hfg, hmul, h1, inner_add_right, inner_smul_right]
  have R1 :
      inner (𝕜 := ℂ) v (gradMinus (H:=H) (fun x => (g x)⁻¹) u)
        = - ((g u)^2)⁻¹ * inner (𝕜 := ℂ) v (gradMinus (H:=H) g u) := by
    simpa [hinv, inner_smul_right, pow_two, inv_pow]
  have RHS :
      inner (𝕜 := ℂ)
        v (((g u)^2)⁻¹ •
          ((g u) • gradMinus (H:=H) f u - (f u) • gradMinus (H:=H) g u))
        =
        (g u)⁻¹ * inner (𝕜 := ℂ) v (gradMinus (H:=H) f u)
        + -(f u) * ((g u)^2)⁻¹ * inner (𝕜 := ℂ) v (gradMinus (H:=H) g u) := by
    -- expand RHS
    have := by
      simp [inner_smul_right, inner_add_right, sub_eq_add_neg,
            pow_two, inv_pow, mul_comm, mul_left_comm, mul_assoc,
            inv_mul_cancel hz, mul_inv_cancel hz]
    exact this
  -- done
