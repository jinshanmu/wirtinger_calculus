...
by
  ...
  refine { toLinearMap := J V
         , cont := ?_ } ;
  -- continuity of `v ↦ I • v`
  -- Use that scalar multiplication is continuous and `I` is constant.
  have h : Continuous fun p : ℂ × V => p.1 • p.2 := continuous_smul
  have h' : Continuous fun v : V => ((I : ℂ), v) := continuous_const.prod_mk continuous_id
  simpa [J, Function.comp] using h.comp h'
...
@[simp] lemma Jc_comp_Jc_apply (V : Type _) [AddCommGroup V] [Module ℂ V]
  ...
  -- `(I • (I • v)) = (I*I) • v = (-1 : ℂ) • v = -v`
  simp [Jc, ContinuousLinearMap.comp_apply, smul_smul, Complex.I_mul_I, neg_one_smul]

-- Shorthands needed later
...
@[simp] lemma Aℒ_apply (T : H →L[ℝ] W) (v : H) :
  ...
  -- Unfold via composition; don’t rely on definal `rfl`.
  simp [Aℒ, Jc, ContinuousLinearMap.comp_apply, smul_smul]
...
lemma Aℒ_involutive (T : H →L[ℝ] W) :
  ...
  -- Prove pointwise.
  ext v
  -- Expand both layers of `Aℒ`
  simp [Aℒ, Jc, ContinuousLinearMap.comp_apply, smul_smul, Complex.I_mul_I]
  -- Now goal is `(-1 : ℂ) • T ((-1 : ℂ) • v) = T v`
  -- Replace inner `(-1:ℂ)•v` with `-v`, then use real-linearity of `T`.
  have h_inner : ((-1 : ℂ) : ℂ) • v = -v := by simpa [neg_one_smul]
  have hT : T ((-1 : ℂ) • v) = - T v := by simpa [h_inner] using T.map_neg v
