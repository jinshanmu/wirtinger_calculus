...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- `D (I • v) = I • D v` from the commutation with `Jc`
      have hJ : D (I • v) = I • D v := by
        have h := congrArg (fun (L : H →L[ℝ] ℂ) => L v)
                    (isCLinearR_Dplus (H:=H) (W:=ℂ) f u)
        -- `Jc_ℂ (D v) = D (Jc_H v)` ⇒ `I • D v = D (I • v)`
        simpa [Jc_apply, ContinuousLinearMap.comp_apply] using h.symm
      -- Decompose `a • v` using re/im and assemble with real linearity + `hJ`.
      have hsplit : a • v = (a.re : ℝ) • v + (a.im : ℝ) • (I • v) := by
        -- Standard decomposition of complex scalar multiplication on a complex module
        simpa [Complex.smul_def]
      calc
        D (a • v)
            = D ((a.re : ℝ) • v + (a.im : ℝ) • (I • v)) := by simpa [hsplit]
        _   = (a.re : ℝ) • D v + (a.im : ℝ) • D (I • v) := by
                simpa [D.map_smul] using
                  (D.map_add ((a.re : ℝ) • v) ((a.im : ℝ) • (I • v)))
        _   = (a.re : ℝ) • D v + (a.im : ℝ) • (I • D v) := by simpa [hJ]
        _   = a • D v := by
                -- same decomposition on the target `ℂ`
                simpa [Complex.smul_def] } ,
...
, map_smul' := by
    ...
    -- From anti-commutation: `Jc ∘ D = - D ∘ Jc`
    -- evaluate at `v` and normalize to get `D (I • v) = -I • D v`.
    have hI : D (I • v) = -I • D v := by
      ...
      have h' := congrArg Neg.neg
                    (by simpa [Jc_apply, ContinuousLinearMap.comp_apply] using h)
      -- `- (I • D v) = D (I • v)` ⇒ desired form
      simpa [Jc_apply, ContinuousLinearMap.comp_apply, neg_neg, neg_smul] using h'
    -- decompose as before, using real linearity of `star` and `hI`
    have hsplit : a • v = (a.re : ℝ) • v + (a.im : ℝ) • (I • v) := by
      simpa [Complex.smul_def]
    have hmap :
        D ((a.re : ℝ) • v + (a.im : ℝ) • (I • v))
          = (a.re : ℝ) • D v + (a.im : ℝ) • D (I • v) := by
      simpa [D.map_smul] using
        (D.map_add ((a.re : ℝ) • v) ((a.im : ℝ) • (I • v)))
    calc
      star (D (a • v))
          = star ((a.re : ℝ) • D v + (a.im : ℝ) • D (I • v)) := by
              simpa [hsplit, hmap]
