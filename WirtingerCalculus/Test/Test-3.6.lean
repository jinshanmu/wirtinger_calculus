...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      -- define an ℝ-linear continuous map G := star ∘ D to ease real-scalar manipulations
      let G : H →L[ℝ] ℂ :=
      { toLinearMap :=
...
/-! ## Wrappers matching the LaTeX statements pointwise -/
...
-- Shorthands as in the prose
abbrev J_H : H →L[ℝ] H := Jc H
abbrev J_W : W →L[ℝ] W := Jc W

/-- (LaTeX (2)) Directional/Gâteaux form:
`deriv (t ↦ f(u + t • v)) 0 = Df[u] v`. -/
...
/-- (LaTeX adjoint, complex-linear) `⟪x, A y⟫ = ⟪A† x, y⟫`. -/
...
/-- (LaTeX adjoint, conjugate-linear; starred form)
`⟪x, B y⟫ = star ⟪B† x, y⟫`, where `B†` is the conjugate-linear adjoint constructed here. -/
@[simp] lemma adjoint_antilinear_star
  ...
  inner (𝕜 := ℂ) x (B y)
    = star (inner (𝕜 := ℂ) ((ConjAdj.conjAdjoint B h_add h_smul h_cont) x) y) :=
  ConjAdj.inner_eq_star_adjoint (B:=B) h_add h_smul h_cont x y

/-- (LaTeX conjugation push) If `C` is isometric and anti-commutes with `J`,
then `⟪C x, C y⟫ = star ⟪x, y⟫ = ⟪y, x⟫`. -/
