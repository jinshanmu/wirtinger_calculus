...
lemma Conjugation.isometry (hC : C.IsAntiunitary) : Isometry C.C := by
  ...
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    intro v
    -- Take `re` of ⟪C v, C v⟫ = ⟪v, v⟫
    have hinner := C.inner_conj_conj_swap hC v v
    rw [norm_sq_eq_re_inner_self, hinner, ← norm_sq_eq_re_inner_self]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  set D := fderivR f u
  -- helper identity for Jc_W ∘ Aℒ
  have h_JAL : (Jc_W).comp (Aℒ (H:=H) (W:=W) D) = - D.comp Jc_H := by
    ext x; simp [Aℒ, Jc_comp_Jc_apply]
  -- helper identity for Aℒ ∘ Jc_H
  have h_AJ : (Aℒ (H:=H) (W:=W) D).comp Jc_H = - (Jc_W).comp D := by
    ext x; simp [Aℒ, Jc_comp_Jc_apply, map_neg]
  -- compute both sides
  simp_rw [DplusCLM, ContinuousLinearMap.comp_smul, ContinuousLinearMap.comp_sub,
    ContinuousLinearMap.sub_comp, ContinuousLinearMap.smul_comp]
  rw [h_JAL, h_AJ, sub_neg_eq_add, sub_neg_eq_add, add_comm]
...
lemma isALinearR_Dminus (f : H → W) (u : H) :
  ...
  have h_AJ : (Aℒ (H:=H) (W:=W) D).comp Jc_H = - (Jc_W).comp D := by
    ext x; simp [Aℒ, Jc_comp_Jc_apply, map_neg]
  -- expand definitions and simplify using helpers
  simp_rw [DminusCLM, ContinuousLinearMap.comp_smul, ContinuousLinearMap.comp_add,
    ContinuousLinearMap.neg_comp, ContinuousLinearMap.add_comp, ContinuousLinearMap.smul_comp]
  rw [h_JAL, h_AJ, sub_eq_add_neg, add_comm]
...
{ toLinearMap :=
  { toFun := fun v => DplusCLM (H:=H) (W:=ℂ) f u v
    ...
    , map_smul' := by
        ...
        -- decompose c = a + bI and use ℝ-linearity + hJ
        let a := c.re
        let b := c.im
        calc D (c • v)
            = D ((a : ℂ) • v + (b • I : ℂ) • v) := by rw [Complex.re_add_im c]
          _ = D ((a : ℝ) • v + (b : ℝ) • (I • v)) := by
              rw [Complex.real_smul_eq_coe_smul, smul_assoc]
          _ = (a : ℝ) • D v + (b : ℝ) • D (I • v) := by simp [map_add]
          _ = (a : ℝ) • D v + (b : ℝ) • (I • D v) := by rw [hJ]
          _ = ((a : ℂ) • D v) + ((b : ℂ) • I • D v) := by simp
          _ = ((a : ℂ) + (b : ℂ) * I) • D v := by rw [smul_assoc, ← add_smul]
          _ = c • D v := by rw [Complex.re_add_im c]
  }
...
{ toFun := fun v => star (DminusCLM (H:=H) (W:=ℂ) f u v)
  ...
  , map_smul' := by
      ...
      have hJ : D ((I : ℂ) • v) = - (I : ℂ) • D v := by
        ...
        simp only [Jc_apply, ContinuousLinearMap.comp_apply] at h
        exact eq_neg_of_eq_neg h
      let a := c.re
      let b := c.im
      calc star (D (c • v))
          = star (D (((a : ℂ) + b • I) • v)) := by rw [Complex.re_add_im c]
        _ = star (D ((a : ℝ) • v + (b : ℝ) • (I • v))) := by
              rw [Complex.real_smul_eq_coe_smul, smul_assoc]
        _ = star ((a : ℝ) • D v + (b : ℝ) • D (I • v)) := by simp [map_add]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (D (I • v)) := by simp [star_add, star_smul]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (-I • D v) := by rw [hJ]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • (I • star (D v)) := by
            simp [star_smul, Complex.star_I, neg_neg]
        _ = ((a : ℂ) + b • I) • star (D v) := by
            rw [Complex.real_smul_eq_coe_smul, Complex.real_smul_eq_coe_smul, smul_assoc, ← add_smul]
            simp
        _ = c • star (D v) := by rw [Complex.re_add_im c]
