...
`⟪A x, y⟫ = ⟪x, A† y⟫`. For **anti-linear** maps we do *not* overload `†`;
instead we provide `conjAdjoint` with the Riesz identity below.
-/
...
`𝒜` is an involution on the space of `ℝ`-linear maps:
`𝒜 (𝒜 T) = T`. Its `+1`-eigenspace consists of anti-linear maps
(anti-commuting with `J`), and its `-1`-eigenspace consists of complex-linear maps
(commuting with `J`). The projections are
`Tplus = (Id - 𝒜)/2` (complex-linear part) and `Tminus = (Id + 𝒜)/2` (anti-linear part).
-/
...
When an inner product is present, being “antiunitary” means it preserves
...
/-- A conjugation is *antiunitary* if it preserves inner products up to complex conjugation:
`⟪C x, C y⟫ = star ⟪x, y⟫`, which is equivalent to `⟪C x, C y⟫ = ⟪y, x⟫`. -/
...
**Setup.** `inner` is conjugate-linear in the first slot and linear in the second (mathlib).
Given a **conjugate-linear** map `B : E → F` (i.e. `B (a • y) = (star a) • B y`) that is
additive and continuous, we define `(conjAdjoint B) : F → E` via the Riesz representation
so that:

* `⟪(conjAdjoint B) x, y⟫ = ⟪B y, x⟫` for all `x : F`, `y : E`;
* equivalently, `⟪x, B y⟫ = star ⟪(conjAdjoint B) x, y⟫`.

**Remark.** Because the first slot of `inner` is conjugate-linear while `B` is conjugate-linear,
the map `y ↦ ⟪B y, x⟫` is ℂ-linear in `y` (conjugation twice gives linearity).
-/

namespace ConjAdj

/-- For fixed `x : F`, the bounded **ℂ-linear** functional `y ↦ ⟪B y, x⟫`.
Linearity in `y` holds because `B` is conjugate-linear and the inner product
...
by
  classical
  -- 1) Underlying linear map:
  -- ℂ-linear in `y` since `B` and the first slot are both conjugate-linear:
  let L : E →ₗ[ℂ] ℂ :=
...
/-- Adjoint of a conjugate-linear `B`, via the Riesz isometry (notated `conjAdjoint`, not `†`). -/
def conjAdjoint
  {E F : Type*}
  [NormedAddCommGroup E] [InnerProductSpace ℂ E] [CompleteSpace E]
  [NormedAddCommGroup F] [InnerProductSpace ℂ F] [CompleteSpace F]
