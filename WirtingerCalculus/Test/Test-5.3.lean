...
lemma gradMinus_mul
  ...
  simpa [star_add, star_mul, inner_conj_symm, inner_add_left, inner_smul_left,
         add_comm, add_left_comm, add_assoc] using h1
...
lemma gradPlus_inv
  ...
  have h_inv :
    ...
    -- use the point version of the derivative and `HasFDerivAt.inv`
  ...
  apply (InnerProductSpace.toDual ℂ H).injective
  ext v
  have hc : star (g u) ≠ 0 := by
    intro h; exact hgu (by simpa using congrArg star h)
...
lemma gradPlus_div
  ...
  calc
    ...
    _   = (star (f u)) • ( - ((star (g u))^2)⁻¹ • gradPlus (H:=H) g u )
            + ((star (g u))⁻¹) • gradPlus (H:=H) f u := by
            simpa [hinv, star_inv]   -- use `star_inv` for `star ((g u)⁻¹)`
    _   = ((star (g u))^2)⁻¹ •
            ( (star (g u)) • gradPlus (H:=H) f u
              - (star (f u)) • gradPlus (H:=H) g u ) := by
      simp [smul_add, smul_sub, sub_eq_add_neg, smul_smul,
