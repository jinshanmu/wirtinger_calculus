...
lemma Dplus_conj_op
  ...
  classical
  -- Put equalities in the exact syntactic form that appears in goals.
  ...
  -- Kernel sometimes writes it as `starRingEnd ℂ`.
  have hE_star : fderiv ℝ (fun x => (starRingEnd ℂ) (f x)) u = conjCLM.comp D := by
    simpa using hE'
  have hD : fderivR (H:=H) (W:=ℂ) f u = D := by
    simpa [HasRDerivAt, fderivR] using hf.fderiv
  -- `Aℒ` pushes through `conjCLM` with a minus sign, purely at operator level.
  have hA :
  ...
  -- Same identity in the unfolded kernel shape.
  have hA' :
      Jc_W.comp ((conjCLM.comp D).comp Jc_H)
        = - conjCLM.comp (Jc_W.comp (D.comp Jc_H)) := by
    simpa [Aℒ] using hA
  -- Compute with `comp_*` algebra, avoiding aggressive `simp`.
  unfold DplusCLM
  calc
    (1 / 2 : ℝ) •
...
lemma gradPlus_conj_eq_gradMinus
  ...
  have h0L :=
    (LinearIsometryEquiv.apply_symm_apply
      ...
      (DplusCLM_c_linear (H:=H) (fun x => star (f x)) u))
  have h1L :
      (InnerProductSpace.toDual ℂ H)
        (gradPlus (H:=H) (fun x => star (f x)) u)
      = DplusCLM_c_linear (H:=H) (fun x => star (f x)) u := by
    simpa [gradPlus] using h0L
  have hL :
      ((InnerProductSpace.toDual ℂ H)
        (gradPlus (H:=H) (fun x => star (f x)) u)) v
      = (DplusCLM_c_linear (H:=H) (fun x => star (f x)) u) v :=
    congrArg (fun T : H →L[ℂ] ℂ => T v) h1L
  -- right side at the dual
  have h0R :=
    (LinearIsometryEquiv.apply_symm_apply
      (InnerProductSpace.toDual ℂ H)
      (DminusCLM_star_c_linear (H:=H) f u))
  have h1R :
      (InnerProductSpace.toDual ℂ H)
        (gradMinus (H:=H) f u)
      = DminusCLM_star_c_linear (H:=H) f u := by
    simpa [gradMinus] using h0R
  have hR :
      ((InnerProductSpace.toDual ℂ H)
        (gradMinus (H:=H) f u)) v
      = (DminusCLM_star_c_linear (H:=H) f u) v :=
    congrArg (fun T : H →L[ℂ] ℂ => T v) h1R
  -- stitch the chain
  calc
    ((InnerProductSpace.toDual ℂ H)
      (gradPlus (H:=H) (fun x => star (f x)) u)) v
        = (DplusCLM_c_linear (H:=H) (fun x => star (f x)) u) v := hL
...
lemma gradMinus_conj_eq_gradPlus
  ...
  have h0L :=
    (LinearIsometryEquiv.apply_symm_apply
      (InnerProductSpace.toDual ℂ H)
      (DminusCLM_star_c_linear (H:=H) (fun x => star (f x)) u))
  have h1L :
      (InnerProductSpace.toDual ℂ H)
        (gradMinus (H:=H) (fun x => star (f x)) u)
      = DminusCLM_star_c_linear (H:=H) (fun x => star (f x)) u := by
    simpa [gradMinus] using h0L
  have hL :
      ((InnerProductSpace.toDual ℂ H)
        (gradMinus (H:=H) (fun x => star (f x)) u)) v
      = (DminusCLM_star_c_linear (H:=H) (fun x => star (f x)) u) v :=
    congrArg (fun T : H →L[ℂ] ℂ => T v) h1L
  -- right side at the dual
  have h0R :=
    (LinearIsometryEquiv.apply_symm_apply
      (InnerProductSpace.toDual ℂ H)
      (DplusCLM_c_linear (H:=H) f u))
  have h1R :
      (InnerProductSpace.toDual ℂ H)
        (gradPlus (H:=H) f u)
      = DplusCLM_c_linear (H:=H) f u := by
    simpa [gradPlus] using h0R
  have hR :
      ((InnerProductSpace.toDual ℂ H)
        (gradPlus (H:=H) f u)) v
      = (DplusCLM_c_linear (H:=H) f u) v :=
    congrArg (fun T : H →L[ℂ] ℂ => T v) h1R
  -- stitch the chain
  calc
    ((InnerProductSpace.toDual ℂ H)
      (gradMinus (H:=H) (fun x => star (f x)) u)) v
        = (DminusCLM_star_c_linear (H:=H) (fun x => star (f x)) u) v := hL
    _   = star (DminusCLM (H:=H) (W:=ℂ) (fun x => star (f x)) u v) := by
          simp [DminusCLM_star_c_linear_apply]
    _ = star (star (DplusCLM (H:=H) (W:=ℂ) f u v)) := by
      simpa using
        congrArg star (Dminus_conj_dir (H:=H) (u:=u) (D:=D) hf v)
