...
lemma Dplus_add_Dminus (f : H → W) (u : H) :
  ...
  simp only [DplusCLM, DminusCLM, ContinuousLinearMap.add_apply, ContinuousLinearMap.smul_apply,
    ContinuousLinearMap.sub_apply, Aℒ_apply]
  rw [smul_sub, smul_add, sub_eq_add_neg, smul_neg, add_add_add_comm, add_right_inv, add_zero,
    ← add_smul, add_halves, one_smul]
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  have h1 : Jc_W.comp (Aℒ D) = -D.comp Jc_H := by
    ext x
    simp only [ContinuousLinearMap.comp_apply, Aℒ_apply, ContinuousLinearMap.neg_apply, Jc_apply]
    rw [smul_smul, I_mul_I, neg_one_smul]
  have h2 : (Aℒ D).comp Jc_H = -Jc_W.comp D := by
    ext x
    simp only [ContinuousLinearMap.comp_apply, Aℒ_apply, ContinuousLinearMap.neg_apply, Jc_apply]
    rw [smul_smul, I_mul_I, neg_one_smul, map_neg, smul_neg]
