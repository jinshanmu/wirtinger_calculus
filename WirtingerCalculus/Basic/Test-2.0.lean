...
* Properties of conjugations, including `Isometry` and `*` notation.
...
-- Notation for conjugation, e.g., u*
scoped notation u"*" => C.C u
...
lemma Conjugation.isometry (hC : C.IsAntiunitary) : Isometry C.C := by
  ...
  -- Show ‖C v‖ = ‖v‖ for all v
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    intro v
    simp [norm_sq_eq_inner, C.inner_conj_conj_swap hC, inner_self_conj_symm]
  intro v
...
def DplusCLM_c_linear (f : H → ℂ) (u : H) : H →L[ℂ] ℂ :=
  { DplusCLM f u with
    map_smul' := by
      ...
      let a : ℝ := re c
      let b : ℝ := im c
      have hc : c = a + b • I := (re_add_im c).symm
      rw [hc, smul_add, ContinuousLinearMap.map_add]
      -- Linearity over a
      have ha : DplusCLM f u (a • v) = a • DplusCLM f u v :=
        ContinuousLinearMap.map_smul (DplusCLM f u) a v
      -- Linearity over b*I
      have hb : DplusCLM f u ((b • I) • v) = (b • I) • DplusCLM f u v := by
        simp_rw [smul_assoc, ← Jc_apply, ContinuousLinearMap.map_smul,
          ← ContinuousLinearMap.comp_apply, isCLinearR_Dplus,
          ContinuousLinearMap.comp_apply, Jc_apply, smul_assoc]
      rw [ha, hb, ← smul_add, ← hc] }
...
@[simp] lemma inner_gradPlus_eq_Dplus (f : H → ℂ) (u : H) (v : H) :
    inner (gradPlus f u) v = DplusCLM f u v := by
...
def DminusCLM_star_c_linear (f : H → ℂ) (u : H) : H →L[ℂ] ℂ :=
  { toFun := fun v => star (DminusCLM f u v)
    ...
    map_smul' := by
      ...
      have hc : c = a + b • I := (re_add_im c).symm
      rw [hc, smul_add, ContinuousLinearMap.map_add]
      -- For a: star (D (a v)) = star (a D v) = a star (D v) = (star a) star (D v)
      have ha : star (DminusCLM f u (a • v)) = star c • star (DminusCLM f u v) := by
        simp [ContinuousLinearMap.map_smul, star_smul]
      -- For b*I: star (D(bIv)) = star (b D(Iv)) = b star(D(Iv)) = b star(-I Dv) = b I star(Dv)
      have hb : star (DminusCLM f u ((b • I) • v)) = (b • I : ℂ) • star (DminusCLM f u v) := by
        simp_rw [smul_assoc, ← Jc_apply, ContinuousLinearMap.map_smul,
          ← ContinuousLinearMap.comp_apply, isALinearR_Dminus,
          ContinuousLinearMap.comp_apply, Jc_apply, map_neg, star_neg, star_smul,
          star_I, neg_smul, neg_neg, smul_assoc]
      rw [hc, star_add, ha, hb, ← smul_add]
    cont := continuous_star.comp (DminusCLM f u).continuous }
...
@[simp] lemma Dminus_eq_inner_gradMinus (f : H → ℂ) (u : H) (v : H) :
    DminusCLM f u v = inner v (gradMinus f u) := by
  rw [← inner_star_eq_swap, star_ring_end_self_apply]
  -- Goal is: `star (DminusCLM f u v) = inner (gradMinus f u) v`
...
lemma fderivR_apply_split_grad (f : H → ℂ) (u v : H) :
    fderivR f u v = inner (gradPlus f u) v + inner v (gradMinus f u) := by
  rw [← inner_gradPlus_eq_Dplus, ← Dminus_eq_inner_gradMinus, fderivR_apply_split]
