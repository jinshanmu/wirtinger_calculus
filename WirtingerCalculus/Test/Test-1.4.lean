...
by
  classical
  refine (J V).mkContinuous (1 : ℝ) ?_
  intro v
  -- ‖I • v‖ = ‖I‖ ‖v‖ = 1⋅‖v‖
  have : ‖(I : ℂ) • v‖ = ‖(I : ℂ)‖ * ‖v‖ := norm_smul _ _
  have hI : ‖(I : ℂ)‖ = 1 := by simpa using norm_I
  simpa [J, hI, one_mul] using (le_of_eq this)
...
@[simp] lemma Jc_comp_Jc_apply (V : Type _) [AddCommGroup V] [Module ℂ V]
  ...
  simp [ContinuousLinearMap.comp_apply, Jc, smul_smul, Complex.I_mul_I, neg_one_smul]
