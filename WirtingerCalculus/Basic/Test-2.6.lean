...
lemma Conjugation.isometry (C : Conjugation V) (hC : C.IsAntiunitary) : Isometry C.C := by
  ...
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    ...
    rw [norm_sq_eq_re_inner, hinner, ← norm_sq_eq_re_inner]
...
{ toLinearMap :=
  { toFun := fun v => DplusCLM (H:=H) (W:=ℂ) f u v
    ...
    , map_smul' := by
        ...
        let a := c.re
        let b := c.im
        -- The action of a complex scalar c = a + ib can be decomposed into
        -- ℝ-linear actions, which is what D respects.
        have h_smul_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
          simp only [re_add_im c, add_smul, smul_assoc, ←ofReal_smul]
        -- Now we apply D and use its ℝ-linearity and its commutation with J (hJ).
        rw [h_smul_decomp, D.map_add, D.map_smul, D.map_smul, hJ, ←smul_assoc, ←add_smul]
        -- Finally, we reconstruct the complex scalar c.
        rw [re_add_im c]
        simp only [ofReal_smul, mul_smul]
  }
...
{ toFun := fun v => star (DminusCLM (H:=H) (W:=ℂ) f u v)
  ...
  , map_smul' := by
      ...
      have h_smul_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
        simp only [re_add_im c, add_smul, smul_assoc, ←ofReal_smul]
      -- Apply star ∘ D and use properties
      rw [h_smul_decomp, D.map_add, D.map_smul, D.map_smul, star_add]
      -- Use star_smul for the real scalars a and b
      rw [star_smul_of_tower, star_smul_of_tower, hJ, star_neg, star_smul]
      -- Use star I = -I and reassemble the scalar c
      rw [(star_def I).trans conj_I, smul_neg, neg_neg, ←smul_assoc, ←add_smul]
      rw [re_add_im c]
      simp only [ofReal_smul, mul_smul]
