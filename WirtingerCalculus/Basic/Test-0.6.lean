...
by
  ...
  , map_add' := by intro y z; simp [h_add y z]
  , map_smul' := by
      intro a y
      simp [h_smul a y, mul_comm] }
  -- Continuity via `toDual` and conjugate symmetry
