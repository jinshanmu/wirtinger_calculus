...
by
  -- Construct directly from the linear map `J V` and continuity of `v ↦ (I:ℂ) • v`.
  refine
    { toLinearMap := J V
      cont := ?_ }
  simpa [J] using (continuous_id.const_smul (I : ℂ))
...
lemma Aℒ_involutive (T : H →L[ℝ] W) :
  ...
  simp [Aℒ, Jc_apply, ContinuousLinearMap.comp_apply, smul_smul, Complex.I_mul_I, neg_one_smul]
  have hT : T (-v) = - T v := by simpa using T.map_neg v
  simpa [hT]
