...
by
  ...
  , map_smul' := by
      ...
      have h' : inner (𝕜 := ℂ) ((star a) • B y) x = a * inner (𝕜 := ℂ) (B y) x := by
        rw [inner_smul_left, star_star, mul_comm]
      simpa [hb] using h' }
  -- Continuity:
...
{ toLinearMap :=
  ...
  , map_smul' := fun a v => by
      ...
      have h_comm : D (I • v) = I • D v :=
        (congr_fun (isCLinearR_Dplus f u) v).symm
      -- Decompose `a • v` using `a = a.re + a.im * I`, then apply D's ℝ-linearity
      -- and its commutation with I, then recombine.
      rw [(Complex.re_add_im a).symm, add_smul, ofReal_smul', mul_smul, ofReal_smul',
        D.map_add, D.map_smul, D.map_smul, h_comm,
        ← ofReal_smul', ← add_smul, ← mul_smul, ← ofReal_smul', Complex.re_add_im a] } ,
...
{ toLinearMap :=
  ...
  , map_smul' := fun a v => by
      ...
      have h_anti_comm : D (I • v) = -I • D v := by
        have h0 : I • D v = -D (I • v) := congr_fun (isALinearR_Dminus f u) v
        rwa [eq_neg_iff_eq_neg, neg_neg]
      -- Goal: star (D (a • v)) = a • star (D v)
      -- Expand both sides using a = a.re + a.im * I
      -- LHS:
      rw [(Complex.re_add_im a).symm, add_smul, ofReal_smul', mul_smul, ofReal_smul']
      rw [D.map_add, D.map_smul, D.map_smul, h_anti_comm]
      rw [star_add, star_smul_of_real, star_smul_of_real, star_smul, star_neg_I,
        smul_smul, neg_smul, smul_neg, neg_neg]
      -- RHS:
      rw [ofReal_smul', add_smul, mul_smul, ofReal_smul', ← (Complex.re_add_im a).symm] } ,
