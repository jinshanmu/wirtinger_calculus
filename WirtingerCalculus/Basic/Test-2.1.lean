...
`⟪A† x, y⟫ = ⟪x, A y⟫` (equivalently, `⟪A x, y⟫ = ⟪x, A† y⟫`).
...
`⟪A† x, y⟫ = ⟪x, A y⟫` under mathlib’s convention
...
section
variable {V : Type _} [AddCommGroup V] [Module ℂ V] (C : Conjugation V)

@[simp] lemma Conjugation.comp_self_id :
    C.C.comp C.C = LinearMap.id := C.involutive

/-- Pointwise form of `C ∘ J = - J ∘ C`. -/
@[simp] lemma Conjugation.antiJ_apply (v : V) :
...
variable {V : Type _}
variable [NormedAddCommGroup V] [InnerProductSpace ℂ V] (C : Conjugation V)
...
lemma Conjugation.isometry (hC : C.IsAntiunitary) : Isometry C.C := by
  -- Show distances are preserved.
  refine Isometry.of_dist_eq (fun x y => ?_)
  -- First, show squared norms are preserved.
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    intro v
    -- Take `IsROrC.re` of ⟪C v, C v⟫ = ⟪v, v⟫
    have hinner := C.inner_conj_conj_swap hC v v
    simpa [norm_sq_eq_inner] using congrArg IsROrC.re hinner
...
def Jc (V : Type _) [NormedAddCommGroup V] [NormedSpace ℂ V] : V →L[ℝ] V :=
  { toLinearMap := J V,
    cont := (continuous_id.const_smul (I : ℂ)) }
...
@[simp] lemma Jc_comp_Jc_apply (V : Type _) [NormedAddCommGroup V] [NormedSpace ℂ V]
  ...
  simp [ContinuousLinearMap.comp_apply, Jc_apply, smul_smul, Complex.I_mul_I, neg_one_smul]
...
lemma Dplus_add_Dminus (f : H → W) (u : H) :
  ...
  ext v
  simp only [DplusCLM, DminusCLM, ContinuousLinearMap.add_apply,
...
lemma isCLinearR_Dplus (f : H → W) (u : H) :
  ...
  set D := fderivR f u
  -- helper identities
  have h1 : Jc_W (Aℒ (H:=H) (W:=W) D x) = - D (Jc_H x) := by
    -- Aℒ D x = Jc_W (D (Jc_H x)) ; apply Jc_W once more
    simp [Aℒ, ContinuousLinearMap.comp_apply, Jc_apply, smul_smul,
          Complex.I_mul_I, neg_one_smul]
  have h2 : (Aℒ (H:=H) (W:=W) D) (Jc_H x) = - Jc_W (D x) := by
    -- Aℒ D (Jc_H x) = Jc_W (D (Jc_H (Jc_H x))) = Jc_W (D (-x)) = - Jc_W (D x)
    simp [Aℒ, ContinuousLinearMap.comp_apply, Jc_apply, smul_smul,
          Complex.I_mul_I, map_neg]
  -- compute both sides
  simp [DplusCLM, Aℒ, ContinuousLinearMap.comp_apply, ContinuousLinearMap.sub_apply,
        ContinuousLinearMap.smul_apply, h1, h2]
...
lemma isALinearR_Dminus (f : H → W) (u : H) :
  ...
  have h2 : (Aℒ (H:=H) (W:=W) D) (Jc_H x) = - Jc_W (D x) := by
    simp [Aℒ, ContinuousLinearMap.comp_apply, Jc_apply, smul_smul,
          Complex.I_mul_I, map_neg]
  -- left: (1/2)(Jc D x + Jc Aℒ D x); right: -(1/2)(D Jc x + Aℒ D Jc x)
  -- use h1,h2 to match terms
  simp [DminusCLM, Aℒ, ContinuousLinearMap.comp_apply, ContinuousLinearMap.add_apply,
        ContinuousLinearMap.smul_apply, h1, h2, add_comm]
...
{ toLinearMap :=
  { toFun := fun v => DplusCLM (H:=H) (W:=ℂ) f u v
    ...
    , map_smul' := by
        intro c v
        classical
        -- decompose c into real/imag parts
        set a : ℝ := Complex.re c
        set b : ℝ := Complex.im c
        have hc : c = (a : ℂ) + b • I := by
          simpa [a, b, re_add_im] using (re_add_im c).symm
        -- pointwise commutation: D₊(I•v) = I • D₊ v
        have hJ : DplusCLM (H:=H) (W:=ℂ) f u ((I : ℂ) • v)
                    = (I : ℂ) • DplusCLM (H:=H) (W:=ℂ) f u v := by
          -- from (Jc_W ∘ D₊) = (D₊ ∘ Jc_H), evaluated at v
          have := congrArg (fun (L : H →L[ℝ] ℂ) => L v)
                    (isCLinearR_Dplus (H:=H) (W:=ℂ) f u)
          -- Jc_W (D₊ v) = D₊ (Jc_H v)  ⇒  I•D₊ v = D₊ (I•v)
          simpa [Jc_apply, ContinuousLinearMap.comp_apply, smul_smul] using this.symm
        -- expand c•v and compute
        have hsplit : c • v = (a : ℂ) • v + (b • I : ℂ) • v := by
          simpa [hc, add_smul]
        calc
          DplusCLM (H:=H) (W:=ℂ) f u (c • v)
              = DplusCLM (H:=H) (W:=ℂ) f u ((a : ℂ) • v + (b • I : ℂ) • v) := by
                  simpa [hsplit]
          _ = DplusCLM (H:=H) (W:=ℂ) f u ((a : ℝ) • v)
              + DplusCLM (H:=H) (W:=ℂ) f u ((b : ℝ) • ((I : ℂ) • v)) := by
                  simpa [map_add, smul_smul]
          _ = (a : ℝ) • DplusCLM (H:=H) (W:=ℂ) f u v
              + (b : ℝ) • DplusCLM (H:=H) (W:=ℂ) f u ((I : ℂ) • v) := by
                  simp [ContinuousLinearMap.map_smul]
          _ = (a : ℝ) • DplusCLM (H:=H) (W:=ℂ) f u v
              + (b : ℝ) • ((I : ℂ) • DplusCLM (H:=H) (W:=ℂ) f u v) := by
                  simpa [hJ]
          _ = ((a : ℂ) + b • I) • DplusCLM (H:=H) (W:=ℂ) f u v := by
                  simp [add_smul, smul_smul]
          _ = c • DplusCLM (H:=H) (W:=ℂ) f u v := by simpa [hc]
  }
...
{ toFun := fun v => star (DminusCLM (H:=H) (W:=ℂ) f u v)
  ...
  , map_smul' := by
      ...
      have hc : c = (a : ℂ) + b • I := by
        simpa [a, b, re_add_im] using (re_add_im c).symm
      have hJ : DminusCLM (H:=H) (W:=ℂ) f u ((I : ℂ) • v)
                  = - (I : ℂ) • DminusCLM (H:=H) (W:=ℂ) f u v := by
        have := congrArg (fun (L : H →L[ℝ] ℂ) => L v)
                  (isALinearR_Dminus (H:=H) (W:=ℂ) f u)
        -- Jc_W (D₋ v) = - D₋ (Jc_H v) ⇒ D₋(I•v) = -(I)•D₋ v
        simpa [Jc_apply, ContinuousLinearMap.comp_apply, smul_smul] using this
      have hsplit : c • v = (a : ℂ) • v + (b • I : ℂ) • v := by
        simpa [hc, add_smul]
      calc
        star (DminusCLM (H:=H) (W:=ℂ) f u (c • v))
            = star (DminusCLM (H:=H) (W:=ℂ) f u
                  ((a : ℂ) • v + (b • I : ℂ) • v)) := by simpa [hsplit]
        _ = star (DminusCLM (H:=H) (W:=ℂ) f u ((a : ℝ) • v)
                  + DminusCLM (H:=H) (W:=ℂ) f u ((b : ℝ) • ((I : ℂ) • v))) := by
                  simpa [map_add, smul_smul]
        _ = star ((a : ℝ) • DminusCLM (H:=H) (W:=ℂ) f u v
                  + (b : ℝ) • DminusCLM (H:=H) (W:=ℂ) f u ((I : ℂ) • v)) := by
                  simp [ContinuousLinearMap.map_smul]
        _ = (a : ℝ) • star (DminusCLM (H:=H) (W:=ℂ) f u v)
            + (b : ℝ) • star (DminusCLM (H:=H) (W:=ℂ) f u ((I : ℂ) • v)) := by
                  simp [star_add, star_smul]
        _ = (a : ℝ) • star (DminusCLM (H:=H) (W:=ℂ) f u v)
            + (b : ℝ) • ((I : ℂ) • star (DminusCLM (H:=H) (W:=ℂ) f u v)) := by
                  -- star(D₋(I•v)) = star(-(I)•D₋ v) = star(-I)•star(D₋ v) = I•star(D₋ v)
                  simpa [hJ, star_neg, star_smul, star_I, neg_neg]
        _ = ((a : ℂ) + b • I) • star (DminusCLM (H:=H) (W:=ℂ) f u v) := by
                  simp [add_smul, smul_smul]
        _ = c • star (DminusCLM (H:=H) (W:=ℂ) f u v) := by simpa [hc]
...
@[simp] lemma Dminus_eq_inner_gradMinus (f : H → ℂ) (u v : H) :
  ...
  -- First show: inner (gradMinus f u) v = star (D₋ f u v)
  ...
  -- Take `star` and use `inner_star_eq_swap`.
...
This matches the definition of the adjoint for anti-linear operators in the text.
...
by
  classical
  -- Underlying ℂ-linear map:
  let L : E →ₗ[ℂ] ℂ :=
  { toFun := fun y => inner (𝕜 := ℂ) (B y) x
  , map_add' := by
      intro y z; simp [h_add y z]
  , map_smul' := by
      intro a y
      -- first slot is conjugate-linear, and B picks up a `star a`
      -- so overall we get `a` in front
      simp [h_smul a y, mul_comm] }
  -- Continuity: y ↦ ⟪B y, x⟫ = star ⟪x, B y⟫, composition of continuous maps
  ...
  have hLcont : Continuous fun y : E => L y := by
    -- Convert `hstar` to the goal using conjugate symmetry pointwise.
