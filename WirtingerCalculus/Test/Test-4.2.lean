...
/-!
Disable the global `[simp]` that expands `conj z` into `re/im`.
It causes `fderivR (fun x => star (f x))` to be simplified into a
`re/imag` expression and breaks the operator calculations below.
-/
attribute [-simp] star_eq_re_sub_im
...
lemma gradPlus_conj_eq_gradMinus
  ...
  classical
  -- Compare on the dual side and evaluate at arbitrary `v`.
  ...
  calc
    ((InnerProductSpace.toDual ℂ H)
      (gradPlus (H:=H) (fun x => star (f x)) u)) v
        = (DplusCLM_c_linear (H:=H) (fun x => star (f x)) u) v := by
          simp [gradPlus]   -- allow `apply_symm_apply` etc.
    ...
    _   = ((InnerProductSpace.toDual ℂ H) (gradMinus (H:=H) f u)) v := by
          simp [gradMinus]   -- unfold and simplify to the dual
...
lemma gradMinus_conj_eq_gradPlus
  ...
  calc
    ...
    _   = star (star (DplusCLM (H:=H) (W:=ℂ) f u v)) := by
          simpa using Dminus_conj_dir (H:=H) (u:=u) (D:=D) hf v
    _   = DplusCLM (H:=H) (W:=ℂ) f u v := by simp
    _   = ((InnerProductSpace.toDual ℂ H) (gradPlus (H:=H) f u)) v := by
          simp [gradPlus, DplusCLM_c_linear_apply]
