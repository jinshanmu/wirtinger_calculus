...
`⟪C x, C y⟫ = star ⟪x, y⟫`. With mathlib’s convention this is equivalent to `⟪C x, C y⟫ = ⟪y, x⟫`. -/
...
by
  classical
  -- 1) Underlying linear map (ℂ-linear in `y` since `B` and the first slot are both conjugate-linear):
  let L : E →ₗ[ℂ] ℂ :=
