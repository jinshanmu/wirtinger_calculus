...
/-- Exact printed form: `a • v = a.re • v + a.im • I • v`. -/
lemma complexSmul_decompose {V : Type _} [AddCommGroup V] [Module ℂ V]
  ...
  calc
    ...
    _   = (a.re : ℂ) • v + ((a.im : ℂ) * I) • v := by
              -- precise rewrite for (x + y) • v
              simpa using (add_smul (a.re : ℂ) ((a.im : ℂ) * I) v)
    _   = (a.re : ℝ) • v + a.im • I • v := by
              -- turn ℂ-scalar and (ℝ⋅I) forms into the statement’s target
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      have hv : a • v = (a.re : ℝ) • v + a.im • I • v :=
        complexSmul_decompose a v
      have hR :
        D (a • v) = a.re • D v + a.im • D (I • v) := by
        have := congrArg D hv
        simp [D.map_add, D.map_smul] at this
        exact this
      ...
      -- 3) Substitute `hI` and factor with explicit reassociation
      ...
      have hfact :
        ...
        -- turn the second summand into `((a.im : ℂ) * I) * D v`
        have : (a.im : ℂ) * (I * D v) = ((a.im : ℂ) * I) * D v := by
          simp [mul_assoc]
        -- then use `(x + y) * z = x*z + y*z` backwards
...
{ toLinearMap :=
  ...
  , map_smul' := by
      ...
      have hI_G : I * G v = G (I • v) := by
        ...
        -- commutativity of ℂ multiplication is used here
      ...
      have hR :
        ...
        have := congrArg G hv
        simp [G.map_add, G.map_smul] at this
        exact this
      ...
      -- 3) Substitute `hI_G` and factor with explicit reassociation
      have hR_mul' :
