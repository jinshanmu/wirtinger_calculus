...
* A handy identity: `D₊ f u + D₋ f u = fderivR f u` (as CLMs).
...
@[simp] lemma ofReal_mul_I (r : ℝ) : (r : ℂ) * I = I * (r : ℂ) := by
  simpa [mul_comm]   -- commutativity in `ℂ`
...
/-!  The total derivative splits as the sum of ± parts (operator identity). -/
lemma Dplus_add_Dminus (f : H → ℂ) (u : H) :
  D₊ f u + D₋ f u = fderivR f u := by
  classical
  ext v
