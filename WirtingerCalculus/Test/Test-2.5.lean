...
lemma Conjugation.isometry (C : Conjugation V) (hC : C.IsAntiunitary) : Isometry C.C := by
  ...
  have h_norm_sq : ∀ v, ‖C.C v‖^2 = ‖v‖^2 := by
    ...
    have hinner := Conjugation.inner_conj_conj_swap C hC v v
  ...
  { toFun := fun v => DplusCLM (H:=H) (W:=ℂ) f u v
    ...
    , map_smul' := by
        ...
        have h_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
          simp only [hc, add_smul, smul_assoc, ←coe_real_smul]
        calc D (c • v)
            = D ((a : ℝ) • v + (b : ℝ) • (I • v))       := by rw [h_decomp]
          _ = D.map_add ((a : ℝ) • v) ((b : ℝ) • (I•v)) := rfl
          _ = D ((a : ℝ) • v) + D ((b : ℝ) • (I • v))   := by rw [D.map_add]
          _ = (a : ℝ) • D v + (b : ℝ) • D (I • v)       := by simp_rw [D.map_smul]
          _ = (a : ℝ) • D v + (b : ℝ) • (I • D v)       := by rw [hJ]
          _ = ((a : ℂ) • D v) + ((b : ℂ) • (I • D v))   := by simp_rw [coe_real_smul]
          _ = ((a : ℂ) • D v) + (((b : ℂ) * I) • D v)   := by rw [smul_assoc]
          _ = ((a : ℂ) + (b : ℂ) * I) • D v             := by rw [← add_smul]
          _ = c • D v                                   := by rw [hc]
  }
...
{ toFun := fun v => star (DminusCLM (H:=H) (W:=ℂ) f u v)
  ...
  , map_smul' := by
      ...
      have hJ : D ((I : ℂ) • v) = - (I : ℂ) • D v := by
        ...
        rw [eq_comm, neg_eq_iff_eq_neg] at h; exact h
      let a := c.re
      let b := c.im
      have hc : c = (a : ℂ) + (b : ℂ) * I := (re_add_im c).symm
      have h_decomp : c • v = (a : ℝ) • v + (b : ℝ) • (I • v) := by
      ...
      calc star (D (c • v))
        ...
        _ = star (D ((a : ℝ) • v) + D ((b : ℝ) • (I • v))) := by rw [D.map_add]
        _ = star ((a : ℝ) • D v + (b : ℝ) • D (I • v))     := by simp_rw [D.map_smul]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (D (I • v)) := by simp only [star_add, star_smul]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • star (-I • D v) := by rw [hJ]
        _ = (a : ℝ) • star (D v) + (b : ℝ) • (I • star (D v)) := by
            simp only [star_smul, star_neg, Complex.star_I, neg_neg]
        _ = ((a : ℂ) • star (D v)) + ((b : ℂ) • I • star (D v)) := by simp_rw [coe_real_smul]
        _ = ((a : ℂ) + (b : ℂ) * I) • star (D v)           := by rw [smul_assoc, ← add_smul]
